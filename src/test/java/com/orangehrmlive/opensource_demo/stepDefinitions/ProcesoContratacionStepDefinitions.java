...
public class ProcesoContratacionStepDefinitions {
    ...
    @Cuando("inicia sesión y se visualiza la pagina de {string}")
    public void iniciaSesiónYSeVisualizaLaPaginaDe(String textValidation) {
    ...
    @Cuando("{string} selecciona reclutamiento y hace clic en el botón agregar un candidato")
    public void seleccionaReclutamientoYHaceClicEnElBotónAgregarUnCandidato(String actor) {
        OnStage.theActorCalled(actor).wasAbleTo(AgregarCandidatoTask.addRecruitment());
    }

    @Cuando("{string} ingresa los datos del candidato con información válida")
    public void ingresaLosDatosDelCandidatoConInformaciónVálida(String actor) {
